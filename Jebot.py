...
async def song(client, message):
  ...
  # also splitting text for correct yt search
    ...
    status = await message.reply(
             text="<b>සිංදුව Download වෙමින් පවතී,චුට්ටක් ඉන්නකො 🥺\n\nMade by @EZTee_channel </b>",
    ...
    try:
        ...
        await status.edit("<b>සිංදුව download කිරිමට නොහැකි වූවා 🤕</b>")
        LOGGER.error(ex)
        return ""
...
async def song(client, message):
    ...
    if args.startswith(" "):
        ...
        return ""
    status = await message.reply(
             text="<b>සිංදුව Download වෙන ගමන් ටිකක් ඉන්නකෝ🥺\n\nMade by @EZTee_channel </b>",
             disable_web_page_preview=True,
                        reply_markup=InlineKeyboardMarkup(
                            [[
                                    InlineKeyboardButton(
                                        "Developer", url=" https://t.me/EZ_Tee ")
                                ]]
                        ),
               parse_mode="html",
        reply_to_message_id=message.message_id
...
async def start(client, message):
   if message.chat.type == 'private':
       await Jebot.send_message(
               chat_id=message.chat.id,
               text="""<b>Hey There,මම සිංදු ඩව්න්ලොඩ් කරල දෙනව🙂♥
...
Hit help button to find out more about how to use me</b>""",   
                            reply_markup=InlineKeyboardMarkup(
                                [[
                                        ...
                                        InlineKeyboardButton(
                                            "Channel", url="https://t.me/EzTee_channel ")
                                    ]]
                            ),        
            disable_web_page_preview=True,        
            parse_mode="html",
